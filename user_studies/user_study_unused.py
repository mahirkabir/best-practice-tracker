...
def get_unused_deps(repo, tag):
    """Get unused dependencies for `tag` of `repo`"""
    reader = open(os.path.join("results", "unused_dup_checker_all_tags",
                               repo, tag, "unused.txt"), "r", encoding="utf-8")
...
def generate_issue(template, repo, tag, url, report):
    ...
    template = template.replace("<UNUSED DEPENDENCIES>", report)
...
if __name__ == "__main__":
    ...
    study_repos = []
    reader = open(os.path.join(".", "user_studies", "unused_repos.txt"), "r")
    lines = reader.readlines()
    reader.close()
    for line in lines:
    ...
    reader = open(os.path.join("user_studies",
                               "unused_deps_template.txt"), "r", encoding="utf-8")
    lines = reader.readlines()
    reader.close()
    for line in lines:
        template += line
    ...
    for repo in study_repos:
        try:
            report = get_unused_deps(repo, dict_repo_latest_tag[repo])
            generated_issue = generate_issue(
                template, repo, dict_repo_latest_tag[repo], dict_repo_url[repo], report)

            writer = open(os.path.join("user_studies", "unused_issues",
